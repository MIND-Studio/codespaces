 ...
 * Because it's a dumb pipe, prod can point `NEXT_PUBLIC_COLLAB_RELAY_URL` at the
 * shared whiteboard relay instead of running this — the room namespace keeps the
 * two apps' docs from colliding.
 *
 * What it is:
 *   • A bare `ws` server speaking the y-websocket wire protocol (sync +
 *     awareness). One in-memory Y.Doc + Awareness per room (= room id, taken
 ...
 *     and the durable copy (the pod / the committed `.mind` issue) is untouched;
 *     restart and clients re-sync from each other + their IndexedDB cache. The
 *     room doc is GC'd when the last peer leaves.
 *   • NO pod credentials. The relay never talks to any Solid pod.
 *   • NO decryption. The relay is a dumb pipe; it never inspects payloads.
 *
...
const PORT = Number(process.env.RELAY_PORT ?? 3012);
...
const server = http.createServer((req, res) => {
  ...
  res.end("mind-codespaces collab relay — ephemeral y-websocket broker. No data stored.\n");
});
...
server.listen(PORT, HOST, () => {
  ...
  console.log(
    `[relay] mind-codespaces collab relay on ws://${HOST}:${PORT} (no persistence, no pod creds)`,
  );
});
